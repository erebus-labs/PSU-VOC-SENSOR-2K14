...
/* `#END` */
    ...
    ******************************************************************************/
    ...
    {
        #if (defined(USB_UART_ENABLE_MIDI_STREAMING) && USB_UART_ISR_SERVICE_MIDI_OUT && CY_PSOC3)
            uint8 int_en;
        #endif /* USB_UART_ISR_SERVICE_MIDI_OUT && CY_PSOC3 */
        ...
        CY_GET_REG8(USB_UART_SIE_EP1_CR0_PTR); /* Must read the mode reg */
        /* Do not toggle ISOC endpoint */
        if((USB_UART_EP[USB_UART_EP1].attrib & USB_UART_EP_TYPE_MASK) !=
                                                                                    USB_UART_EP_TYPE_ISOC)
        {
            USB_UART_EP[USB_UART_EP1].epToggle ^= USB_UART_EPX_CNT_DATA_TOGGLE;
        }
        USB_UART_EP[USB_UART_EP1].apiEpState = USB_UART_EVENT_PENDING;
        CY_SET_REG8(USB_UART_SIE_EP_INT_SR_PTR, CY_GET_REG8(USB_UART_SIE_EP_INT_SR_PTR) &
                                                                    (uint8)~USB_UART_SIE_EP_INT_EP1_MASK);

        #if( defined(USB_UART_ENABLE_MIDI_STREAMING) && USB_UART_ISR_SERVICE_MIDI_OUT )
            if(USB_UART_midi_out_ep == USB_UART_EP1)
            {
                USB_UART_MIDI_OUT_EP_Service();
            }
        #endif /* End USB_UART_ISR_SERVICE_MIDI_OUT */
    ...
    *******************************************************************************/
    CY_ISR(USB_UART_EP_2_ISR)
    {
        ...
        CY_GET_REG8(USB_UART_SIE_EP2_CR0_PTR); /* Must read the mode reg */
        /* Do not toggle ISOC endpoint */
        if((USB_UART_EP[USB_UART_EP2].attrib & USB_UART_EP_TYPE_MASK) !=
                                                                                    USB_UART_EP_TYPE_ISOC)
        {
        ...
        CY_SET_REG8(USB_UART_SIE_EP_INT_SR_PTR, CY_GET_REG8(USB_UART_SIE_EP_INT_SR_PTR)
                                                                        & (uint8)~USB_UART_SIE_EP_INT_EP2_MASK);
    ...
    *******************************************************************************/
    CY_ISR(USB_UART_EP_3_ISR)
    {
        ...
        CY_GET_REG8(USB_UART_SIE_EP3_CR0_PTR); /* Must read the mode reg */
        /* Do not toggle ISOC endpoint */
        if((USB_UART_EP[USB_UART_EP3].attrib & USB_UART_EP_TYPE_MASK) !=
                                                                                    USB_UART_EP_TYPE_ISOC)
        {
        ...
        CY_SET_REG8(USB_UART_SIE_EP_INT_SR_PTR, CY_GET_REG8(USB_UART_SIE_EP_INT_SR_PTR)
                                                                        & (uint8)~USB_UART_SIE_EP_INT_EP3_MASK);
    ...
    *******************************************************************************/
    CY_ISR(USB_UART_EP_4_ISR)
    {
        ...
        CY_GET_REG8(USB_UART_SIE_EP4_CR0_PTR); /* Must read the mode reg */
        /* Do not toggle ISOC endpoint */
        if((USB_UART_EP[USB_UART_EP4].attrib & USB_UART_EP_TYPE_MASK) !=
                                                                                    USB_UART_EP_TYPE_ISOC)
        {
        ...
        CY_SET_REG8(USB_UART_SIE_EP_INT_SR_PTR, CY_GET_REG8(USB_UART_SIE_EP_INT_SR_PTR)
                                                                        & (uint8)~USB_UART_SIE_EP_INT_EP4_MASK);
    ...
    *******************************************************************************/
    CY_ISR(USB_UART_EP_5_ISR)
    {
        ...
        CY_GET_REG8(USB_UART_SIE_EP5_CR0_PTR); /* Must read the mode reg */
        /* Do not toggle ISOC endpoint */
        if((USB_UART_EP[USB_UART_EP5].attrib & USB_UART_EP_TYPE_MASK) !=
                                                                                    USB_UART_EP_TYPE_ISOC)
        {
        ...
        CY_SET_REG8(USB_UART_SIE_EP_INT_SR_PTR, CY_GET_REG8(USB_UART_SIE_EP_INT_SR_PTR)
                                                                        & (uint8)~USB_UART_SIE_EP_INT_EP5_MASK);
    ...
    *******************************************************************************/
    CY_ISR(USB_UART_EP_6_ISR)
    {
        ...
        CY_GET_REG8(USB_UART_SIE_EP6_CR0_PTR); /* Must read the mode reg */
        /* Do not toggle ISOC endpoint */
        if((USB_UART_EP[USB_UART_EP6].attrib & USB_UART_EP_TYPE_MASK) !=
                                                                                    USB_UART_EP_TYPE_ISOC)
        {
        ...
        CY_SET_REG8(USB_UART_SIE_EP_INT_SR_PTR, CY_GET_REG8(USB_UART_SIE_EP_INT_SR_PTR)
                                                                        & (uint8)~USB_UART_SIE_EP_INT_EP6_MASK);
    ...
    *******************************************************************************/
    CY_ISR(USB_UART_EP_7_ISR)
    {
        ...
        CY_GET_REG8(USB_UART_SIE_EP7_CR0_PTR); /* Must read the mode reg */
        /* Do not toggle ISOC endpoint */
        if((USB_UART_EP[USB_UART_EP7].attrib & USB_UART_EP_TYPE_MASK) !=
                                                                                    USB_UART_EP_TYPE_ISOC)
        {
        ...
        CY_SET_REG8(USB_UART_SIE_EP_INT_SR_PTR, CY_GET_REG8(USB_UART_SIE_EP_INT_SR_PTR)
                                                                        & (uint8)~USB_UART_SIE_EP_INT_EP7_MASK);
    ...
    *******************************************************************************/
    CY_ISR(USB_UART_EP_8_ISR)
    {
        ...
        CY_GET_REG8(USB_UART_SIE_EP8_CR0_PTR); /* Must read the mode reg */
        /* Do not toggle ISOC endpoint */
        if((USB_UART_EP[USB_UART_EP8].attrib & USB_UART_EP_TYPE_MASK) !=
                                                                                    USB_UART_EP_TYPE_ISOC)
        {
        ...
        CY_SET_REG8(USB_UART_SIE_EP_INT_SR_PTR, CY_GET_REG8(USB_UART_SIE_EP_INT_SR_PTR)
                                                                        & (uint8)~USB_UART_SIE_EP_INT_EP8_MASK);
...
CY_ISR(USB_UART_BUS_RESET_ISR)
{
    ...
    *******************************************************************************/
    CY_ISR(USB_UART_ARB_ISR)
    {
        ...
        int_status = USB_UART_ARB_INT_SR_REG;                   /* read Arbiter Status Register */
        USB_UART_ARB_INT_SR_REG = int_status;                   /* Clear Serviced Interrupts */

        while(int_status != 0u)
        {
            if((int_status & 1u) != 0u)  /* If EpX interrupt present */
            {   /* read Endpoint Status Register */
                ep_status  = CY_GET_REG8((reg8 *)(USB_UART_ARB_EP1_SR_IND + ptr));
                /* If In Buffer Full */
                if((ep_status & USB_UART_ARB_EPX_SR_IN_BUF_FULL) != 0u)
                {
                    if((USB_UART_EP[ep].addr & USB_UART_DIR_IN) != 0u)
                    {
                        /* Clear Data ready status */
                        *(reg8 *)(USB_UART_ARB_EP1_CFG_IND + ptr) &=
                                                                    (uint8)~USB_UART_ARB_EPX_CFG_IN_DATA_RDY;
                        /* Write the Mode register */
                        CY_SET_REG8((reg8 *)(USB_UART_SIE_EP1_CR0_IND + ptr), USB_UART_EP[ep].epMode);
                ...
                /* (re)arm Out EP only for mode2 */
                    ...
                    /* If DMA Grant */
                    if((ep_status & USB_UART_ARB_EPX_SR_DMA_GNT) != 0u)
                    {
                        if((USB_UART_EP[ep].addr & USB_UART_DIR_IN) == 0u)
                        {
                                USB_UART_EP[ep].apiEpState = USB_UART_NO_EVENT_PENDING;
                                /* Write the Mode register */
                                CY_SET_REG8((reg8 *)(USB_UART_SIE_EP1_CR0_IND + ptr),
                                                                                    USB_UART_EP[ep].epMode);
                        }
                    }
                ...
                CY_SET_REG8((reg8 *)(USB_UART_ARB_EP1_SR_IND + ptr), ep_status);   /* Clear Serviced events */
            }
            ptr += USB_UART_EPX_CNTX_ADDR_OFFSET;               /* prepare pointer for next EP */
            ep++;
            int_status >>= 1u;
        }
