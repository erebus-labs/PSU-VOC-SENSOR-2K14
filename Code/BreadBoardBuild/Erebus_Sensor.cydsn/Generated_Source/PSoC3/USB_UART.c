...
#if(USB_UART_DMA1_REMOVE == 0u)
    #include "USB_UART_ep1_dma.h"
#endif   /* End USB_UART_DMA1_REMOVE */
#if(USB_UART_DMA2_REMOVE == 0u)
    #include "USB_UART_ep2_dma.h"
#endif   /* End USB_UART_DMA2_REMOVE */
#if(USB_UART_DMA3_REMOVE == 0u)
    #include "USB_UART_ep3_dma.h"
#endif   /* End USB_UART_DMA3_REMOVE */
#if(USB_UART_DMA4_REMOVE == 0u)
    #include "USB_UART_ep4_dma.h"
#endif   /* End USB_UART_DMA4_REMOVE */
#if(USB_UART_DMA5_REMOVE == 0u)
    #include "USB_UART_ep5_dma.h"
#endif   /* End USB_UART_DMA5_REMOVE */
#if(USB_UART_DMA6_REMOVE == 0u)
    #include "USB_UART_ep6_dma.h"
#endif   /* End USB_UART_DMA6_REMOVE */
#if(USB_UART_DMA7_REMOVE == 0u)
    #include "USB_UART_ep7_dma.h"
#endif   /* End USB_UART_DMA7_REMOVE */
#if(USB_UART_DMA8_REMOVE == 0u)
    #include "USB_UART_ep8_dma.h"
#endif   /* End USB_UART_DMA8_REMOVE */
...
*        following table.
*       USB_UART_3V_OPERATION - Disable voltage regulator and pass-thru
*                                       Vcc for pull-up
*       USB_UART_5V_OPERATION - Enable voltage regulator and use
*                                       regulator for pull-up
*       USB_UART_DWR_VDDD_OPERATION - Enable or Disable voltage
...
*  and set to one (1u) the first time USB_UART_Start() is called.
*  This allows for component Re-Start without unnecessary re-initialization
*  in all subsequent calls to the USB_UART_Start() routine.
*  If re-initialization of the component is required the variable should be set
*  to zero before call of UART_Start() routine, or the user may call
*  USB_UART_Init() and USB_UART_InitComponent() as done
*  in the USB_UART_Start() routine.
*
* Side Effects:
...
*
* Summary:
*  Initialize component's hardware. Usually called in USB_UART_Start().
*
* Parameters:
*  None.
*
...
void USB_UART_Init(void) 
{
    ...
    */
    ...
    #if((USB_UART_EP_MM != USB_UART__EP_MANUAL) && (USB_UART_ARB_ISR_REMOVE == 0u))
...
*       in the "Device Number" field.
*   USB_UART_transferState: This variable used by the communication
*       functions to handle current transfer state. Initialized to
*       TRANS_STATE_IDLE in this API.
*   USB_UART_configuration: Contains current configuration number
*       which is set by the Host using SET_CONFIGURATION request.
*       Initialized to zero in this API.
*   USB_UART_deviceAddress: Contains current device address. This
    ...
    */
    ...
    switch(mode)
    {
        ...
        case USB_UART_5V_OPERATION:
            USB_UART_CR1_REG = USB_UART_CR1_ENABLE_LOCK | USB_UART_CR1_REG_ENABLE;
            break;
        default:   /*USB_UART_DWR_VDDD_OPERATION */
            #if(USB_UART_VDDD_MV < USB_UART_3500MV)
                USB_UART_CR1_REG = USB_UART_CR1_ENABLE_LOCK;
            #else
                USB_UART_CR1_REG = USB_UART_CR1_ENABLE_LOCK | USB_UART_CR1_REG_ENABLE;
            #endif /* End USB_UART_VDDD_MV < USB_UART_3500MV */
            break;
    }
    ...
    CY_SET_REG8(USB_UART_CR0_PTR, USB_UART_CR0_ENABLE);
...
*       (DEVICE_STATUS_BUS_POWERED or DEVICE_STATUS_SELF_POWERED) and remote
*       wakeup status (DEVICE_STATUS_REMOTE_WAKEUP) in second bit.
*   USB_UART_configurationChanged: This variable is set to one after
*       SET_CONFIGURATION request and cleared in this function.
*   USB_UART_intiVar variable is set to zero
*
*******************************************************************************/
...
uint8 USB_UART_CheckActivity(void) 
{
    ...
    r = CY_GET_REG8(USB_UART_CR1_PTR);
    CY_SET_REG8(USB_UART_CR1_PTR, (r & ((uint8)(~USB_UART_CR1_BUS_ACTIVITY))));

    return((r & USB_UART_CR1_BUS_ACTIVITY) >> USB_UART_CR1_BUS_ACTIVITY_SHIFT);
}
...
uint16 USB_UART_GetEPCount(uint8 epNumber) 
{
    ...
    {
        ri = ((epNumber - USB_UART_EP1) << USB_UART_EPX_CNTX_ADDR_SHIFT);

        result = (uint8)(CY_GET_REG8((reg8 *)(USB_UART_SIE_EP1_CNT0_IND + ri)) &
                          USB_UART_EPX_CNT0_MASK);
        result = (result << 8u) | CY_GET_REG8((reg8 *)(USB_UART_SIE_EP1_CNT1_IND + ri));
        result -= USB_UART_EPX_CNTX_CRC_COUNT;
    }
    return(result);
}
    ...
    *******************************************************************************/
                                                                    ...
                                                                    
    {
        ...
        switch(epNumber)
        {
            case USB_UART_EP1:
                #if(USB_UART_DMA1_REMOVE == 0u)
                    USB_UART_DmaChan[epNumber] = USB_UART_ep1_DmaInitialize(
                        USB_UART_DMA_BYTES_PER_BURST, USB_UART_DMA_REQUEST_PER_BURST, src, dst);
                #endif   /* End USB_UART_DMA1_REMOVE */
                break;
            case USB_UART_EP2:
                #if(USB_UART_DMA2_REMOVE == 0u)
                    USB_UART_DmaChan[epNumber] = USB_UART_ep2_DmaInitialize(
                        USB_UART_DMA_BYTES_PER_BURST, USB_UART_DMA_REQUEST_PER_BURST, src, dst);
                #endif   /* End USB_UART_DMA2_REMOVE */
                break;
            case USB_UART_EP3:
                #if(USB_UART_DMA3_REMOVE == 0u)
                    USB_UART_DmaChan[epNumber] = USB_UART_ep3_DmaInitialize(
                        USB_UART_DMA_BYTES_PER_BURST, USB_UART_DMA_REQUEST_PER_BURST, src, dst);
                #endif   /* End USB_UART_DMA3_REMOVE */
                break;
            case USB_UART_EP4:
                #if(USB_UART_DMA4_REMOVE == 0u)
                    USB_UART_DmaChan[epNumber] = USB_UART_ep4_DmaInitialize(
                        USB_UART_DMA_BYTES_PER_BURST, USB_UART_DMA_REQUEST_PER_BURST, src, dst);
                #endif   /* End USB_UART_DMA4_REMOVE */
                break;
            case USB_UART_EP5:
                #if(USB_UART_DMA5_REMOVE == 0u)
                    USB_UART_DmaChan[epNumber] = USB_UART_ep5_DmaInitialize(
                        USB_UART_DMA_BYTES_PER_BURST, USB_UART_DMA_REQUEST_PER_BURST, src, dst);
                #endif   /* End USB_UART_DMA5_REMOVE */
                break;
            case USB_UART_EP6:
                #if(USB_UART_DMA6_REMOVE == 0u)
                    USB_UART_DmaChan[epNumber] = USB_UART_ep6_DmaInitialize(
                        USB_UART_DMA_BYTES_PER_BURST, USB_UART_DMA_REQUEST_PER_BURST, src, dst);
                #endif   /* End USB_UART_DMA6_REMOVE */
                break;
            case USB_UART_EP7:
                #if(USB_UART_DMA7_REMOVE == 0u)
                    USB_UART_DmaChan[epNumber] = USB_UART_ep7_DmaInitialize(
                        USB_UART_DMA_BYTES_PER_BURST, USB_UART_DMA_REQUEST_PER_BURST, src, dst);
                #endif   /* End USB_UART_DMA7_REMOVE */
                break;
            case USB_UART_EP8:
                #if(USB_UART_DMA8_REMOVE == 0u)
                    USB_UART_DmaChan[epNumber] = USB_UART_ep8_DmaInitialize(
                        USB_UART_DMA_BYTES_PER_BURST, USB_UART_DMA_REQUEST_PER_BURST, src, dst);
                #endif   /* End USB_UART_DMA8_REMOVE */
                break;
            default:
    ...
    *           USB_UART_MAX_EP to stop all DMAs
    *
    * Return:
    *  None.
    *
                                                                        ...
                                                                        
{
    ...
    {
        ri = ((epNumber - USB_UART_EP1) << USB_UART_EPX_CNTX_ADDR_SHIFT);
        p = (reg8 *)(USB_UART_ARB_RW1_DR_IND + ri);
        ...
        CY_SET_REG8((reg8 *)(USB_UART_SIE_EP1_CNT0_IND + ri),
                            (length >> 8u) | (USB_UART_EP[epNumber].epToggle));
        CY_SET_REG8((reg8 *)(USB_UART_SIE_EP1_CNT1_IND + ri),  length & 0xFFu);
            ...
            CY_SET_REG8((reg8 *)(USB_UART_SIE_EP1_CR0_IND + ri), USB_UART_EP[epNumber].epMode);
        #else
            /* Init DMA if it was not initialized */
            if(USB_UART_DmaTd[epNumber] == DMA_INVALID_TD)
            {
            ...
            if((pData != NULL) && (length > 0u))
            {
                ...
                (void) CyDmaTdSetConfiguration(USB_UART_DmaTd[epNumber], length, CY_DMA_DISABLE_TD,
                                                                                 TD_TERMIN_EN | TD_INC_SRC_ADR);
                (void) CyDmaTdSetAddress(USB_UART_DmaTd[epNumber],  LO16((uint32)pData), LO16((uint32)p));
                /* Enable the DMA */
                ...
                * (reg8 *)(USB_UART_ARB_EP1_CFG_IND + ri) |= USB_UART_ARB_EPX_CFG_DMA_REQ;
                * (reg8 *)(USB_UART_ARB_EP1_CFG_IND + ri) &= ((uint8)(~USB_UART_ARB_EPX_CFG_DMA_REQ));
            ...
            else
            {
                ...
                if(length > 0u)
                {
                    ...
                    * (reg8 *)(USB_UART_ARB_EP1_CFG_IND + ri) |= USB_UART_ARB_EPX_CFG_IN_DATA_RDY;
...
*  USB_UART_GetEPState to see if an event is pending.
*
* Parameters:
*  epNumber: Contains the data endpoint number.
*            Valid values are between 1 and 8.
                                                                        ...
                                                                        
{
    ...
    if((epNumber > USB_UART_EP0) && (epNumber < USB_UART_MAX_EP) && (pData != NULL))
    {
        ri = ((epNumber - USB_UART_EP1) << USB_UART_EPX_CNTX_ADDR_SHIFT);
        p = (reg8 *)(USB_UART_ARB_RW1_DR_IND + ri);
            ...
            xferCount = USB_UART_GetEPCount(epNumber);
            if (length > xferCount)
            {
                length = xferCount;
            }
            ...
            (void) CyDmaTdSetAddress(USB_UART_DmaTd[epNumber],  LO16((uint32)p), LO16((uint32)pData));
            /* Enable the DMA */
            (void) CyDmaChSetInitialTd(USB_UART_DmaChan[epNumber], USB_UART_DmaTd[epNumber]);
            (void) CyDmaChEnable(USB_UART_DmaChan[epNumber], 1u);
            ...
            * (reg8 *)(USB_UART_ARB_EP1_CFG_IND + ri) &= ((uint8)(~USB_UART_ARB_EPX_CFG_DMA_REQ));
            ...
            /* Enable DMA in mode3 for transferring data */
            (void) CyDmaChDisable(USB_UART_DmaChan[epNumber]);
            (void) CyDmaTdSetConfiguration(USB_UART_DmaTd[epNumber], length, USB_UART_DmaTd[epNumber],
                                                                                TD_TERMIN_EN | TD_INC_DST_ADR);
            (void) CyDmaTdSetAddress(USB_UART_DmaTd[epNumber],  LO16((uint32)p), LO16((uint32)pData));
...
*
* Global variables:
*  USB_UART_EP[epNumber].apiEpState - set to NO_EVENT_PENDING
*
* Reentrant:
*  No.
*
...
{
    CY_SET_REG8(USB_UART_USBIO_CR0_PTR, bState);
}
...
uint8 USB_UART_GetEPAckState(uint8 epNumber) 
{
    ...
    {
        ri = ((epNumber - USB_UART_EP1) << USB_UART_EPX_CNTX_ADDR_SHIFT);
        cr = CY_GET_REG8((reg8 *)(USB_UART_SIE_EP1_CR0_IND + ri)) & USB_UART_MODE_ACKD;
    }
...
*
* Parameters:
*  powerStatus: USB_UART_DEVICE_STATUS_BUS_POWERED(0) - Bus Powered,
*               USB_UART_DEVICE_STATUS_SELF_POWERED(1) - Self Powered
*
* Return:
*   None.
*
* Global variables:
*  USB_UART_deviceStatus - set power status
*
* Reentrant:
*  No.
*
...
void USB_UART_SetPowerStatus(uint8 powerStatus) 
{
    ...
    *******************************************************************************/
    ...
    {
        return((0u != (CY_GET_REG8(USB_UART_VBUS_PS_PTR) & USB_UART_VBUS_MASK)) ? 1u : 0u);
    }

#endif /* USB_UART_MON_VBUS */
...
uint8 USB_UART_RWUEnabled(void) 
{
    ...
    if((USB_UART_deviceStatus & USB_UART_DEVICE_STATUS_REMOTE_WAKEUP) != 0u)
    {
